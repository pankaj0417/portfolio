...
const Contact = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/sendmessage`, {
        method: "POST",
  ...
  return (
    <div id="contact" className="bg-white dark:bg-gray-800 dark:text-gray-300 flex justify-center items-center mx-10 mt-20 py-12 transition-all duration-500">
      <div className="w-full max-w-lg p-8">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
          {statusMessage && (
            <div
              className={`text-center text-sm font-medium px-4 py-2 rounded ${
                statusType === "success" ? "text-green-600 bg-green-100" : "text-red-600 bg-red-100"
              }`}
            >
              {statusMessage}
            </div>
